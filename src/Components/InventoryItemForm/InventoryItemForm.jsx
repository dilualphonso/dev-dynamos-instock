...
function InventoryItemForm({ itemToEdit }) {
  ...
  function cancelClickHandler(event) {
    ...
    if (!itemToEdit) {
      ...
      // TODO Ilaria  navigate to the correct place for a cancel/edit page (back to the item detail page /inventory/id)
    }
  }
  ...
  return (
    <section className="item-form">
      ...
      >
        <fieldset
          className="item-form__details-container"
          form="submitItemForm"
          name="itemDetailsFields"
        >
          ...
          {isItemNameError && (
            <label className="item-form__error" htmlFor="itemName">
              <img
                src={errorFlag}
                className="item-form__errorIcon"
                alt="A small red-orange circle with a white exclamation mark inside it. Indicates an error in the form."
              />
          ...
          <label className="item-form__label" htmlFor="category">
            Category
          </label>
          <select
            className={
              isCategoryError
                ? "item-form__dropdown item-form__dropdown--error"
                : "item-form__dropdown"
            }
            type="text"
            name="category"
          ...
          >
            ...
            <option className="item-form__dropdown-option" value="Accessories">
              Accessories
            </option>
            <option className="item-form__dropdown-option" value="Apparel">
              Apparel
            </option>
            <option className="item-form__dropdown-option" value="Electronics">
              Electronics
            </option>
            <option className="item-form__dropdown-option" value="Gear">
              Gear
            </option>
            <option className="item-form__dropdown-option" value="Health">
              Health
            </option>
          </select>
          {isCategoryError && (
        ...
        <fieldset
          className="item-form__availability-container"
          form="submitItemForm"
          name="itemAvailabilityFields"
        >
          {/* Use h3 instead of legend in tablet as the styling of fieldset would required non-flex styling */}
          <h3 className="item-form__sub-heading">Item Availability</h3>

          <p className="item-form__label item-form__radio-label--heading">
            Status
          </p>
          <div className="item-form__radio-field-container">
            <div className="item-form__radio-option-container">
              <input
                className="item-form__radioBtn"
                type="radio"
                name="status"
                id="in_stock"
                onChange={inputChangeHandler}
                value="In Stock"
                checked={formInputs.status === "In Stock"}
              />
              <label className="item-form__radio-label" htmlFor="in_stock">
                In stock
              </label>
            </div>
            <div className="item-form__radio-option-container">
              <input
                ...
                onChange={inputChangeHandler}
                value="Out of Stock"
                checked={formInputs.status === "Out of Stock"}
              />
              <label className="item-form__radio-label" htmlFor="out_of_stock">
                Out of stock
              </label>
            </div>
          </div>
          ...
          <div
            className={
              formInputs.status === "In Stock"
                ? "item-form__conditional-field item-form__conditional-field--show"
                : "item-form__conditional-field"
            }
          >
          ...
          <label className="item-form__label" htmlFor="warehouse">
            Warehouse
          </label>
          <WarehouseDropdown
            selected={formInputs.warehouse}
            onSelect={updateForm}
            hasError={isWarehouseError}
          />
          {isWarehouseError && (
        ...
        <div className="item-form__button-container">
          ...
          <button className="item-form__button item-form__button--submit">
            {submitBtnText}
          </button>
        </div>
      </form>
